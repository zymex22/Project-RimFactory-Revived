...
namespace ProjectRimFactory.Industry
{
    public class Building_FuelingMachine : Building
    {
        ...
        protected IEnumerable<Thing> AllThingsForFueling
        {
            get
            {
                ...
                foreach (var item in cell.AllThingsInCellForUse(Map))
                    yield return item;
            }
        }
        ...
        public void Refuel()
        {
            ...
            foreach (var tmpThing in Map.thingGrid.ThingsListAt(FuelableCell))
            {
                    ...
                    if (refuelableComp != null && refuelableComp.Fuel + 0.9999f < refuelableComp.TargetFuelLevel)
                        foreach (var item in AllThingsForFueling)
                            if (refuelableComp.Props.fuelFilter.Allows(item))
                            {
                                ...
                                var num = Mathf.Min(item.stackCount,
                                    Mathf.FloorToInt(refuelableComp.TargetFuelLevel - refuelableComp.Fuel));
                                if (num > 0)
                                {
                                ...
                                if (refuelableComp.Fuel + 0.9999f >= refuelableComp.TargetFuelLevel)
                                    goto Fueled; // fully fueled
                            }
                } // end if is Building

                Fueled:
                //This is a generalized system for mod compatibility - mostlyfor Mortar like buildings
                foreach (var rr in registeredRefuelables) // basically, if (tmpThing is Building_From_rr)
                    if (rr.buildingType.IsAssignableFrom(tmpThing.GetType()))
                    {
                        var o = rr.objectThatNeedsFueling(tmpThing as Building);
        ...
        protected class RegisteredRefuelable
        {
            ...
            public RegisteredRefuelable(Type b, Func<Building, object> nf, Func<object, Thing, int> test,
                Action<object, Thing> a)
            {
                buildingType = b;
    ...
    // -- Pick your own name! --
    internal static class RegisterVanillaMortarsAsRefuelable
    {
        static RegisterVanillaMortarsAsRefuelable()
        {
            ...
            //                               Action< object, Thing > refuelAction);
            refueler.GetMethod("RegisterRefuelable", BindingFlags.Static |
        ...
        private static object FindCompNeedsShells(Building b)
        {
