    ...
    public class Building_PaperclipDuplicator : Building
    {
            ...
            get
            {
                ...
                if (paperclipCount != long.MaxValue)
                    try
                    {
                        checked
                        {
                            result = (long) (paperclipCount * Math.Pow(1.05,
                                (Find.TickManager.TicksGame - lastTick).TicksToDays()));
                        }
                    }
        ...
        public override string GetInspectString()
        {
            ...
            if (boundStorageUnit != null)
                builder.AppendLine("PaperclipsInStorageUnit".Translate(boundStorageUnit.StoredItems
                    .Where(t => t.def == PRFDefOf.Paperclip).Sum(t => t.stackCount)));
            else
                ...
                yield return new Command_Action
                {
                    ...
                    action = () =>
                    {
                        try
                        {
                            checked
                            {
                                if (PaperclipsActual != long.MaxValue) PaperclipsActual = PaperclipsActual * 2;
                            }
                        }
                        catch (OverflowException)
                        {
