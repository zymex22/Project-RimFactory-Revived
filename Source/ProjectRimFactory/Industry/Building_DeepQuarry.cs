...
namespace ProjectRimFactory.Industry
{
    ...
    {
        private static IEnumerable<ThingDef> cachedPossibleRockDefCandidates;
        public CompFlickable flick;
        public CompRefuelable fuel;
        public CompPowerTrader power;
        public int ProducedChunksTotal;
        protected int productionTime; // number of ticks this has been running since last production check
        ...
        protected static IEnumerable<ThingDef> PossibleRockDefCandidates
        {
            get
            {
                if (cachedPossibleRockDefCandidates != null) return cachedPossibleRockDefCandidates;
        ...
        /// <param name="consumeFuelWhileRunning">
        ///     whether or not to manually consume fuel.
            ...
            if (flick == null || flick.SwitchIsOn) // Either no switch or turned on
                if (power == null || power.PowerOn) // Either does not use power or has power
                {
                    if (fuel != null) // uses fuel
                    {
                        ...
                        fuel.ConsumeFuel(fuel.Props.fuelConsumptionRate / 6000 * numTicks);
                        if (fuel.HasFuel) TryGenerateResource(numTicks);
                    }
        ...
        {
            GenPlace.TryPlaceThing(GetChunkThingToPlace(), GetComp<CompOutputAdjustable>().CurrentCell, Map,
                ThingPlaceMode.Near);
            ProducedChunksTotal++;
        }
        ...
        {
            var rock = PossibleRockDefCandidates
                .Where(d => !def.GetModExtension<ModExtension_Miner>()?.IsExcluded(d.building.mineableThing) ?? true)
                .RandomElementByWeight(d =>
                    d.building.isResourceRock
                        ? d.building.mineableScatterCommonality * d.building.mineableScatterLumpSizeRange.Average *
                          d.building.mineableDropChance
                        : 3f);

            var bonus = def.GetModExtension<ModExtension_BonusYield>()?.GetBonusYield();
        ...
        public override string GetInspectString()
        {
            ...
            if (!string.IsNullOrEmpty(s)) stringBuilder.AppendLine(s);
        ...
        public override IEnumerable<Gizmo> GetGizmos()
        {
            ...
            yield return new Command_Action
            {
                ...
                action = () =>
                    Log.Message("number of trials : " + trials + Environment.NewLine +
                                Enumerable.Range(0, trials)
                                    ...
                                    .Aggregate("",
                                        (t, i) => t + "def : " + i.Def.defName + " / stackCount : " + i.StackCount +
                                                  " / minedCount : " + i.Count + " / mine rate(%) : " +
                                                  (float) i.Count * 100f / (float) trials + Environment.NewLine))
            };
        }
#endif
    }
