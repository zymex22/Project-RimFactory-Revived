        ...
        public static Thing CalculateDominantIngredient(RecipeDef currentRecipe, IEnumerable<Thing> thingRecord)
        {
            ...
            if (!thingRecord.Any())
            {
                if (currentRecipe.ingredients.Count > 0)
                    Log.Warning(
                        "S.A.L.: Had no thingRecord of items being accepted, but crafting recipe has ingredients. Did you reload a save?");
                return ThingMaker.MakeThing(ThingDefOf.Steel);
            }

            if (stuffs.Any())
            {
                if (currentRecipe.productHasIngredientStuff) return stuffs.OrderByDescending(x => x.stackCount).First();
