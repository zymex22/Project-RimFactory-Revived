...
namespace ProjectRimFactory.SAL3
{
    public static class BaseCountUtility
    {
        public static float CalculateBaseCountFinalised(Thing item, IngredientCount ingredient)
        {
            ...
            if (ShouldUseNutritionMath(item, ingredient)) basecount *= item.GetStatValue(StatDefOf.Nutrition);
            if (item.def.smallVolume) basecount *= 0.05f;
            return basecount;
        }

        public static bool ShouldUseNutritionMath(Thing t, IngredientCount ingredient)
        {
            return t.GetStatValue(StatDefOf.Nutrition) > 0f && !(t is Corpse) &&
                   IngredientFilterHasNutrition(ingredient.filter);
        }
        ...
        public static bool IngredientFilterHasNutrition(ThingFilter filter)
        {
            if (filter != null)
            {
                bool isNutrition(string str)
                {
                    return str == "Foods" || str == "PlantMatter";
                }
                ...
                foreach (var s in categories)
                    if (DefDatabase<ThingCategoryDef>.GetNamed(s).Parents.Select(t => t.defName).Any(isNutrition) ||
                        isNutrition(s))
                        return true;
            }
