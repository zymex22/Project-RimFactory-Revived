...
using RimWorld;
using Verse; // for dynamic method
    ...
    public static class PlaceThingUtility
    {
        ...
        static PlaceThingUtility()
        {
            ...
            var dm = new DynamicMethod("directly call RimWorld.StoreUtility's NoStorageBlockersIn",
                typeof(bool), new[] {typeof(IntVec3), typeof(Map), typeof(Thing)},
            ...
            il.Emit(OpCodes.Call, typeof(StoreUtility).GetMethod("NoStorageBlockersIn",
        ...
        /// <param name="forcePlace">
        ///     If set to <c>true</c>, forces placing. For when
        ///     you absolutely positively have to put it down.
        /// </param>
            ...
            bool forcePlace = false)
        {
            ...
            if (slotGroup != null)
            {
                Debug.Warning(Debug.Flag.PlaceThing,
                    "Placing " + t + " in slotGroup: " + slotGroup.parent + " at " + cell);
                ...
                if (placer.PlaceThingInSlotGroup(t, slotGroup, cell, map)) return true;
                if (forcePlace) goto ForcePlace;
                return false;
            }

            Debug.Warning(Debug.Flag.PlaceThing,
                "Place request: " + placer == null ? "NoPlacer" : placer + " is trying to place " + t + " at " + cell);
            // Search through all items in cell: see if any will absorb
            //   our thing.  If we find a PRF_Building, stop looking and
            //   try to pass it on.
            var cellIsImpassible = false;
            foreach (var otherThing in map.thingGrid.ThingsListAt(cell))
            {
            ...
            SlotGroup slotGroup, IntVec3 cell, Map map)
        {
            ...
            if (CallNoStorageBlockersIn(cell, map, t))
            {
                Debug.Message(Debug.Flag.PlaceThing,
                    "Found NoStorageBlockersIn(" + cell + ", map, " + t + ") - Placing");
                var wasSpawned = t.Spawned;
                if (wasSpawned) t.DeSpawn();
                if (!GenPlace.TryPlaceThing(t, cell, map, ThingPlaceMode.Direct))
                {
            ...
            Debug.Message(Debug.Flag.PlaceThing,
                "There were StorageBlockersIn(" + cell + ", map, " + t + ") - cannot place");
            return false;
        }
            ...
            SlotGroup slotGroup, IntVec3? cell = null)
        {
            ...
            foreach (var c in cell != null
                ? new[] {(IntVec3) cell}.Concat(slotGroup.CellsList.Where(x => x != cell))
                : slotGroup.CellsList)
