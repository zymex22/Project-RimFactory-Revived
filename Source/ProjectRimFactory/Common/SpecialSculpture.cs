...
namespace ProjectRimFactory
{
    ...
    public class SpecialSculpture : IExposable
    {
        ...
        public List<Thing> currentInstances; // item in game
        ...
        public void ExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                var all = ProjectRimFactory_ModComponent.availableSpecialSculptures;
                if (all != null)
                {
        ...
        public static List<SpecialSculpture> LoadAvailableSpecialSculptures(ModContentPack content)
        {
            try
            {
    ...
    internal class Patch_CompGizmosExtraForArt
    {
        private static IEnumerable<Gizmo> Postfix(IEnumerable<Gizmo> __result, ThingComp __instance)
        {
            ...
            yield return new Command_Action
            {
                ...
                defaultDesc =
                    "Make this Sculpture the first Special Sculpture available.  Reloads everything.\n\nTHIS IS VERY SLOW!\n(Project RimFactory)",
                action = delegate
                {
                    // reload graphic assets: (not fast)
                    LoadedModManager.GetMod<ProjectRimFactory_ModComponent>().Content.ReloadContent();
                    ...
                    ProjectRimFactory_ModComponent.availableSpecialSculptures
                        = SpecialSculpture.LoadAvailableSpecialSculptures(
                            LoadedModManager.GetMod<ProjectRimFactory_ModComponent>().Content
                        );
                    foreach (var s in ProjectRimFactory_ModComponent.availableSpecialSculptures)
                        s.Init();
                    var target = ProjectRimFactory_ModComponent.availableSpecialSculptures[0];
