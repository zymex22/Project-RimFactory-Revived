...
namespace ProjectRimFactory.AutoMachineTool
{
    public class Building_BeltSplitter : Building_BeltConveyor
    {
        ...
        [Unsaved] private readonly HashSet<IBeltConveyorLinkable> incomingLinks = new HashSet<IBeltConveyorLinkable>();

        private Dictionary<Rot4, OutputLink> outputLinks = new Dictionary<Rot4, OutputLink>();

        public Building_BeltSplitter()
        {
        ...
        public override IEnumerable<Rot4> ActiveOutputDirections
        {
            get
            {
                foreach (var kvp in outputLinks.Where(kvp => kvp.Value.Active)) yield return kvp.Key;
            }
        }
        ...
        {
            foreach (var c in AllNearbyLinkables()) c.Unlink(this);
            base.DeSpawn(mode);
        ...
        private bool NextDirection(Thing t, out Rot4 newDir)
        {
            ...
            {
                var olink = outputLinks.TryGetValue(dir);
                if (olink != null)
        ...
        private IEnumerable<Rot4> NextDirectionByPriority(Thing t)
        {
            ...
            var prevPriority = outputLinks.TryGetValue(previousDir)?.priority;
                ...
                .GetValues(typeof(DirectionPriority))).Reverse())
            {
                ...
                for (var i = 0; i < 4; i++, nextDir.Rotate(RotationDirection.Counterclockwise))
                    if (nextDir != Rotation.Opposite && // don't look backwards
                        nextDir != previousDir &&
        ...
        protected override IBeltConveyorLinkable OutputBeltAt(IntVec3 location)
        {
            foreach (var kvp in outputLinks)
                if (kvp.Key.FacingCell + Position == location)
                {
        ...
        protected override bool PlaceProduct(ref List<Thing> products)
        {
            ...
            if (WorkInterruption(thing))
            {
                Debug.Message(Debug.Flag.Conveyors, "   But interrupted; failing");
                return true;
            }

            var output = outputLinks.TryGetValue(dest);
            ...
            if (NextDirection(thing, out var tmp))
            {
        ...
        public override void Link(IBeltConveyorLinkable link)
        {
            if (CanLinkTo(link) && link.CanLinkFrom(this))
                if (PositionToRot4(link, out var r))
                {
                    UpdateGraphic();
            ...
            if (CanLinkFrom(link) && link.CanLinkTo(this)) incomingLinks.Add(link);
        }
        ...
        public override void Unlink(IBeltConveyorLinkable unlink)
        {
            ...
            foreach (var k in tmpl) outputLinks.Remove(k);
            UpdateGraphic();
        }
        ...
        public override bool CanLinkTo(IBeltConveyorLinkable otherBeltLinkable, bool checkPosition = true)
        {
            ...
            if (Position + Rotation.FacingCell == otherBeltLinkable.Position ||
                Position + Rotation.RighthandCell == otherBeltLinkable.Position ||
                // Why is there no LefthandCell? Annoying.
                Position + Rotation.Opposite.RighthandCell == otherBeltLinkable.Position)
                return true;
            return false;
        }
        ...
        public override bool CanLinkFrom(IBeltConveyorLinkable otherBeltLinkable, bool checkPosition = true)
        {
            ...
            if (Position + Rotation.Opposite.FacingCell == otherBeltLinkable.Position)
                return true;
            return false;
        }
        ...
        public class OutputLink : IExposable
        {
            ...
            public void ExposeData()
            {
                ...
                Scribe_References.Look(ref parent, "PRFBSL_parent");
            }
            ...
            public bool TryPlace(Thing thing)
            {
                ...
                if (link != null)
                {
                    ...
                    if (link.AcceptsThing(thing, parent))
                    {
            ...
            public void DoThingFilterConfigWindow(Rect r, ref Vector2 sp)
            {
                ...
                ThingFilterUI.DoThingFilterConfigWindow(r, ref sp, filter);
            }
        }
    }
}